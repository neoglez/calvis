...
class CalvisBasic(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        self.female_annotation_dir = os.path.join(self.annotation_dir,'female')
...
class CalvisCMU2DDataset(CalvisBasic):
    ...
    def __init__(self, root_dir,  image_dir, transform=None):
        ...
        for directory in [self.root_dir, self.female_image_dir,
                          self.male_image_dir, self.female_annotation_dir,
        ...
        self.targets = [s.split("_mesh_")[0] + 
                        '_mesh_' + s.split('_mesh_')[1][:-4] + 
                        '_anno.json' for s in self.images]
    ...
    def __getitem__(self, idx):
        ...
        if self.images_genders[idx] == 0:
            imgfile = os.path.join(self.female_image_dir, 
                                    self.images[idx])
            annofile = os.path.join(self.female_annotation_dir, 
                                    self.targets[idx])
        else:
            imgfile = os.path.join(self.male_image_dir, 
                                    self.images[idx])
            annofile = os.path.join(self.male_annotation_dir, 
                                    self.targets[idx])
        
        image = io.imread(imgfile, as_gray=False)
        sample = {'image': image, 'annotations':load_annotation_data(annofile),
                  'imagefile': imgfile, 'annotation_file': annofile}
    ...
    def getFemaleIndxs(self):
        return [i for i, images_gender in 
                enumerate(self.images_genders) if images_gender == 0]
    
    def getMaleIndxs(self):
        return [i for i, images_gender in 
                enumerate(self.images_genders) if images_gender == 1]
###############################################################################
...
if __name__ == "__main__":
    ...
    train_calvis_cmu_2d_dataset = CalvisCMU2DDataset(root_dir=rootDir, 
                                         image_dir = imageDir)
    
    fair_calvis_cmu_2d_dataset = CalvisFairCMU2DDataset(root_dir=rootDir, 
                                         image_dir = imageDir)
    
    # assert that these two datasets do not have the same length
    assert (bool(len(train_calvis_cmu_2d_dataset) & 
